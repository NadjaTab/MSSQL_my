...
go
if exists(select * from sysobjects where name='sp_replace_sum_sred')
begin
  drop procedure sp_replace_sum_sred
end 
go
create PROCEDURE sp_replace_sum_sred @p_id varchar(15), @mm1 int, @yy1 int, @mcod varchar(5) 
AS
...
update constn set s3 = 0 where pers_id = @p_id and mm + 12* yy >= @mm1 + 12* @yy1 - 12 and mm + 12* yy <= @mm1 + 12* @yy1
  and mm>0

update constn set s1 = 0 where pers_id = @p_id and mm + 12* yy >= @mm1 + 12* @yy1 - 12 and mm + 12* yy <= @mm1 + 12* @yy1
  and mm>0

update constn set s2 = 0 where pers_id = @p_id and mm + 12* yy >= @mm1 + 12* @yy1 - 12 and mm + 12* yy <= @mm1 + 12* @yy1
  and mm>0

update constn set calc_sred = 0 where pers_id = @p_id and mm + 12* yy >= @mm1 + 12* @yy1 - 12 and mm + 12* yy <= @mm1 + 12* @yy1
  and mm>0

update constn set fmp_sr_r = 0 where pers_id = @p_id and mm + 12* yy >= @mm1 + 12* @yy1 - 12 and mm + 12* yy <= @mm1 + 12* @yy1
  and mm>0

update constn set calc_sred = isnull(case alim when 0 then 1 else alim end, 1)*(
...
update constn set fmp_sr_r = (
   select isnull(sum(summa),0) from arhiv_ras_fond 
     where code in (select code from sprav where sredn='1') and (fond='15' or code='355' )
     and pers_id=@p_id and mm=constn.mm and yy=constn.yy
   )
where pers_id_2 = @p_id and mm + 12* yy >= @mm1 + 12* @yy1 - 12 and mm + 12* yy <= @mm1 + 12* @yy1
  and mm>0
...
GO
grant execute on sp_replace_sum_sred to AitUsers
go
